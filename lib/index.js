...
/***/ function(module, exports, __webpack_require__) {
	            ...
	            while (1) {
	                switch (_context.prev = _context.next) {
	                    case 0:
	                        ...
	                        _context.next = 3;
	                        return (0, _someCommand2.default)();
	...
	var _ArgParser = __webpack_require__(366);
	
	var _ArgParser2 = _interopRequireDefault(_ArgParser);
	
	var _cli = __webpack_require__(383);
	
	var _cli2 = _interopRequireDefault(_cli);
	
	var _someCommand = __webpack_require__(384);
	
	var _someCommand2 = _interopRequireDefault(_someCommand);
	...
	var argParser = new _ArgParser2.default(_cli2.default);
	var program = argParser.parse(process.argv);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _slicedToArray2 = __webpack_require__(367);
	...
	var chalk = __webpack_require__(374);
	...
	function _underscoreToCamel(word) {
	    return word.replace(/(_[a-z])/g, function (low) {
	        return low[1].toUpperCase();
	    });
	}
	
	function _camelToUnderscore(word) {
	    return word.replace(/([A-Z])/g, function (cap) {
	...
	function _makeOptHash(optionsSpec) {
	    var optHash = {};
	    optionsSpec.forEach(function (opt) {
	        var argName = opt.optName || _scrubOpt(opt.aliases[0]);
	...
	function _selectCommand(cmdList, arg) {
	    ...
	    try {
	        for (var _iterator = cmdList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	            var cmd = _step.value;
	    ...
	    if (!command || command[0] === '_') {
	        cmdObj = cmdList.find(function (cmd) {
	            return cmd.default;
	        });
	        skipArgs = 0;
	    } else {
	        cmdObj = cmdList.find(function (cmd) {
	            return cmd.normalized === normalizeCmd(command);
	        });
	    }
	
	    util.errorAssertNoStack(cmdObj, 'unknown command', 'Unknown command "' + arg + '"\nPossible commands: ' + _.map(cmdList, 'command').join(', '));
	...
	function _optDone(opts, opt, args) {
	    ...
	    if (opt) {
	        if (args.length === 0) {
	            // Maybe check for legality of bool?
	            opts[opt] = true;
	        } else {
	            (function () {
	                var fullArray = [];
	...
	function _parseOpts(args, optsHash) {
	    ...
	    if (args) {
	        args.forEach(function (arg) {
	            var nextOpt = _asOption(arg, optsHash);
	            if (nextOpt) {
	...
	ArgParser.prototype.validateValue = function (optionName, value) {
	    var opt = undefined._optsHash[optionName];
	    util.assert(opt, 'Unknown option "' + optionName + '"');
	    var negated = /^(no-|~|!)/.test(value.trim());
	    if (opt.values) {
	        var bareValue = value.trim().replace(/^(no-|~|!)/, '').toLowerCase();
	        for (var i = 0; i < opt.values.length; i++) {
	            var matchValue = opt.values[i];
	            if (matchValue.toLowerCase() === bareValue) {
	                return (negated ? '!' : '') + matchValue;
	            }
	        }
	        util.assertAlways('Value "' + value + '" not valid for option "' + optionName + '." Allowed values are ' + opt.values);
	    }
	    return value.trim();
	};
	...
	ArgParser.prototype.parse = function parse(argv, continueAfterHelp) {
	    ...
	    if (argv.length < 3 && !_.find(this._cmdsList, function (cmd) {
	        return cmd.default;
	    }) || argv[2] === 'help' || _asOption(argv[2], this._optsHash) === 'help') {
	        ...
	        var _selectCommand2 = _selectCommand(this._cmdsList, argv[2]);
	
	        var _selectCommand3 = (0, _slicedToArray3.default)(_selectCommand2, 2);
	
	        program.command = _selectCommand3[0];
	        skipArgs = _selectCommand3[1];
	
	        var args = argv.slice(2 + skipArgs);
	...
	function _formatAliases(aliases) {
	    var sortedAliases = aliases.slice(0); // Clone the array since sort is modifying
	    return sortedAliases.sort(function (a, b) {
	...
	function _formatOpt(opt) {
	    var str = _formatAliases(opt.aliases);
	    if (opt.args) {
	        str = str + ' ' + opt.args;
	    }
	    return str;
	...
	ArgParser.prototype.getHelp = function getHelp() {
	    var maxCommandLength = Math.max.apply(null, _.map(_.map(this._cmdsList, 'command'), function (c) {
	        return c.length;
	    }));
	    this._cmdsList.forEach(function (cmd) {
	        cmd.help = '    ' + cmd.command + new Array(maxCommandLength - cmd.command.length + 3).join(' ') + cmd.desc;
	    });
	    var commandHelp = 'Commands:\n\n' + _.map(this._cmdsList, 'help').join('\n');
	
	    var indent = 0;
	    this._optsList.forEach(function (opt) {
	        var help = _formatOpt(opt);
	        opt.help = help;
	    ...
	    this._optsList.forEach(function (opt) {
	        var desc = opt.desc;
	        opt.help = '    ' + opt.help + new Array(indent - opt.help.length + 1).join(' ') + desc + _formatValues(opt, indent);
	    });
	    return this._usage + commandHelp + '\n\nOptions:\n' + _.map(this._optsList, 'help').join('\n') + '\n\n' + this._examples;
	};
	
	ArgParser.prototype.showHelp = function showHelp(continueAfterHelp) {
	    var helpStr = this.getHelp();
	    console.log(helpStr);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _isIterable2 = __webpack_require__(368);
	
	var _isIterable3 = _interopRequireDefault(_isIterable2);
	
	var _getIterator2 = __webpack_require__(371);
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';
	var escapeStringRegexp = __webpack_require__(375);
	var ansiStyles = __webpack_require__(376);
	var stripAnsi = __webpack_require__(378);
	var hasAnsi = __webpack_require__(380);
	var supportsColor = __webpack_require__(381);
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';
	var ansiRegex = __webpack_require__(379)();
...
/***/ function(module, exports, __webpack_require__) {
	...
	/**
	 * Created by sgargoli on 12/22/16.
	 */
	...
	exports.default = (0, _asyncToGenerator3.default)(_regenerator2.default.mark(function _callee() {
	  return _regenerator2.default.wrap(function _callee$(_context) {
	    while (1) {
	      switch (_context.prev = _context.next) {
	        case 0:
	          console.log('Some command performed! (Asynchronously, no less.)');
